...
const AddButton = ({onButtonPress}:addButtonProps) => {
  return(
    <TouchableOpacity onPress={onButtonPress} style={{height:60, width:60, borderRadius:30, justifyContent:'center', alignItems:'center', backgroundColor:'#50AF58', position:'absolute', bottom:'5%', right:'5%'}}> 
...
const Home = ({navigation}:props) => {
    ...
    return (
      <View style={{marginTop:15, marginBottom:10}}>
        {item.transaction_type === 'Received' ? (
          <TouchableOpacity activeOpacity={0.8} onPress={()=>navigation.navigate('ChatScreen')} style={{backgroundColor:'#50AF58', width:'80%', borderRadius:10,alignSelf:'flex-start',}}>
          <View style={{flexDirection:'row', padding:10, alignSelf:'flex-start', borderBottomWidth:1, borderBottomColor:'#e7e7e7'}}>
            <View style={{flex:2,}}>
              <Text allowFontScaling={false} style={{fontSize:14, fontWeight:'bold', color:"#ffffff"}} numberOfLines={1}>{item.name}</Text>
              </View>
            ...
            <Text allowFontScaling={false} style={{fontSize:14, fontWeight:'500', color:"#ffffff"}}>{item.date}:{item.message}</Text>
              </View>
            </TouchableOpacity>
          // <View
          //   style={{
          ...
          <TouchableOpacity activeOpacity={0.8} onPress={()=>navigation.navigate('ChatScreen')} style={{backgroundColor:'white', width:'80%', borderRadius:10,alignSelf:'flex-end', }}>
          <View style={{flexDirection:'row', padding:10, borderBottomWidth:1, borderBottomColor:'#e7e7e7'}}>
            <View style={{flex:2}}>
              <Text allowFontScaling={false} style={{fontSize:14, fontWeight:'bold', color:"#111111"}} numberOfLines={1}>{item.name}</Text>
              </View>
              <View style={{flex:1,justifyContent:'center', alignItems:'center'}}>
                <View style={{backgroundColor:'#e7e7e7', paddingVertical:5, paddingHorizontal:15, borderRadius:5, justifyContent:'center', alignItems:'center'}}>
                <Text allowFontScaling={false} style={{fontSize:14, fontWeight:'bold', color:"#111111"}} numberOfLines={1}>{item.amount}</Text>
                </View>
              </View>
              <View style={{flex:1,justifyContent:'center', alignItems:'center'}}>
                <Text allowFontScaling={false} style={{fontSize:14, fontWeight:'bold', color:"#111111"}} numberOfLines={1}>{item.total_amount}</Text>
              </View>
            </View>
            <View style={{padding:10}}>
            <Text allowFontScaling={false} style={{fontSize:14, fontWeight:'500', color:"#111111"}}>{item.date}:{item.message}</Text>
              </View>
            </TouchableOpacity>
          // <View
          //   style={{
  ...
  return (
    <View style={{flex:1}}>
      <View style={{height:55, width:'100%', backgroundColor:'#ffffff', justifyContent:'center', alignItems:'center', borderBottomColor:'#7a7a7a', borderBottomWidth:2}}>
          <Text allowFontScaling={false} style={{fontSize:22, fontWeight:'bold', color:'#111111'}}>SETTLEIT</Text>
      </View>
